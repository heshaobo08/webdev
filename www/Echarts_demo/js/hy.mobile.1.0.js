...
/*
    ...
    * JS获取网站根目录是基于JS库的目录，以hy.mobile文件夹的上级为根目录
        如：/js/hy.mobile/hy.mobile.1.0.js 则/为根目录
...
; window.hm = (function ($) {
    ...
    hm.showMsg = function (content, title, key, buttons, selectindex, showhtml) {
        ...
        weui_mask.addClass("weui-mask");
        weui_dialog.attr("id", key);
        weui_dialog_type.addClass("weui-dialog");
        dialog_title.addClass("weui-dialog__hd");
        dialog_title_content.addClass("weui-dialog__title");
        dialog_content.addClass("weui-dialog__bd");
        dialog_ft.addClass("weui-dialog__ft");
        ...
        if (buttons) {
            weui_dialog.addClass("js_dialog");
            //点击按钮后自动关闭
            $.each(buttons, function (i, event) {
                ...
                btn.addClass("weui-dialog__btn weui-dialog__btn_default");
                dialog_ft.append(btn);
            })
        } else {
            weui_dialog.addClass("js_dialog");
            btn_confirm = $("<a></a>");
            btn_confirm.addClass("weui-dialog__btn weui-dialog__btn_primary");
            btn_confirm.text("确定");
        ...
        $(dialog_ft.children()[selectindex - 1]).removeClass("weui-dialog__btn_default");
        $(dialog_ft.children()[selectindex - 1]).addClass("weui-dialog__btn_primary");
            ...
            this.checkValid = function () {
                ...
                $.each($("" + selecter + " input").not("[data-hy-novalidate]"), function (index, item) {
                    var tag = $(item).attr("data-hy-type"),
                        msg = $(item).attr("data-hy-errmsg"),
                        inputVal = $(item).val();
                    return hm.verifyInput(tag, inputVal, msg, function (ret, msg) {
                        errMsg = msg;
                        if (ret === "invalid_tag") {
                            ...
                            if (valid) {
                                ...
                                if (!$(item).val()) {
                                    errMsg = $(item).attr("data-hy-errmsg") ? $(item).attr("data-hy-errmsg") : item.title + "格式不正确";
                                    valid = false;
                                } else {
                                    valid = true;
                                }
    ...
    hm.verifyInput = function (tag, input, msg, callback) {
    ...
    hm.format = function () {
        ...
        var method = function () {
            ...
            this.dateTimeToString = function (time, format) {
                ...
                if (hm.getClientInfo().os.isIOS && typeof (time) == "string") {
                    var tag = "-",
                        y, m, d, hh, mm, ss;
                    if (time.indexOf("/") > -1) {
                        tag = "/";
                    ...
                    try {
                        y = time.split(" ")[0].split(tag)[0] ? time.split(" ")[0].split(tag)[0] : 0;
                        m = time.split(" ")[0].split(tag)[1] ? time.split(" ")[0].split(tag)[1] : 0;
                        d = time.split(" ")[0].split(tag)[2] ? time.split(" ")[0].split(tag)[2] : 0;
                        hh = time.split(" ")[1].split(":")[0] ? time.split(" ")[1].split(":")[0] : 0;
                        mm = time.split(" ")[1].split(":")[1] ? time.split(" ")[1].split(":")[1] : 0;
                        ss = time.split(" ")[1].split(":")[2] ? time.split(" ")[1].split(":")[2] : 0;

                        m = parseInt(m) - 1;
                        time = new Date(y, m, d, hh, mm, ss);
                    } catch (e) {
                        time = new Date();
                    }
                } else {
    ...
    hm.pageMgr = {
        ...
        pageMgrLoad: function () {
            hm.pageMgr.fastClick();
            hm.pageMgr.androidInputBugFix();
            hm.pageMgr.setPageManager();

            window.pageMgr = hm.pageMgr;
