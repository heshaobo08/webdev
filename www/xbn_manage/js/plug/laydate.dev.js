...
! function (win) {
    ...
    //全局配置，如果采用默认均不需要改动
    var config = {
        path: 'js/plug/', //laydate所在路径 2015-9-15 liuyu path 修改 （添加js/plug/ ）
    ...
    Dates.initDate = function () {
        if(Dates.options.hideData){
            $(".laydate_top,.laydate_table").hide();
        }else{
            $(".laydate_top,.laydate_table").show();
        }
        var S = Dates.query,
            log = {},
            De = new Date();
    ...
    Dates.events = function () {
        ...
        Dates.on(as.oclear, 'click', function () {
            Dates.elem[as.elemv] = '';
            (Dates.options.empty && 　Dates.options.empty());
            Dates.close();
        });
