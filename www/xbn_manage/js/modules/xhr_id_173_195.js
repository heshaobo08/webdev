...
// 获取后台操作管理员
    ctrl.request({
        url: dataPath+ X.configer[request.m].api.userList,
        type: "post",
        data:'{}'
    }).then(function(userdata){
        if(userdata.statusCode=='2000000'){
            if(userdata.data){
                 $.each(userdata.data,function (i,operator) {
                    operatorList[operator.id]=operator;
                });
            }
            ...
            ctrl.request({
                ...
                if(data.statusCode == "2000000"){
                    data.data.operatorList=operatorList;
    ...
    ctrl.on('dom-event-init',function(elem){
		...
		$(".paytimeStart").on("click",function(){
			laydate({
				istime: true,
				elem : '#paystartTime',
				event : 'focus',
	...
	ctrl.on('searchFormValid',function(){
        ...
        $('#searchForm').html5Validate(function(){
            ...
            validate: function() {
                // 开始时间和结束时间的校验
                if($("#startTime").html() && $('#endTime').html()=="" || $("#paystartTime").html() && $('#payendTime').html()==""){
					if($("#startTime").html() && $('#endTime').html()==""){
						$("#endTime").testRemind("请选择结束时间");
					}else{
						$("#payendTime").testRemind("请选择结束时间");
					}
                    return false;
                }else if($("#startTime").html()=="" && $('#endTime').html() || $("#paystartTime").html()=="" && $('#payendTime').html()){
					if($("#startTime").html()=="" && $('#endTime').html()){
						$("#startTime").testRemind("请选择开始时间");
					}else{
						$("#paystartTime").testRemind("请选择开始时间");
					}
                    return false;
                }else if($("#startTime").html()>$('#endTime').html() || $("#paystartTime").html()>$('#payendTime').html()){
					if($("#startTime").html()>$('#endTime').html()){
						...
						$("#paystartTime").testRemind("开始时间不能大于结束时间");
					}
    ...
    ctrl.on('searchSubmit',function(toPageNo,callback){
        var depositUserName = $("[name=depositUserName]").val(),
			payStatus = $("[name=payStatus]").attr("index-data"),
			oprationBy = $("[name=oprationBy]").val(),
			startPlanPayTime = $("#paystartTime").text(),
			endPlanPayTime = $("#payendTime").text(),
			startActualPayTime = $("#startTime").text(),
			endActualPayTime = $("#endTime").text(),
			sendData={
				depositUserName:depositUserName ? depositUserName : "",
				payStatus:payStatus ? payStatus : "",
				oprationBy:oprationBy ? oprationBy : "",
				startPlanPayTime:startPlanPayTime ? startPlanPayTime : "",
				endPlanPayTime:endPlanPayTime ? endPlanPayTime : "",
				startActualPayTime:startActualPayTime ? startActualPayTime : "",
				endActualPayTime:endActualPayTime ? endActualPayTime : "",
				toPageNo:toPageNo,
				pageSize:50
			};
