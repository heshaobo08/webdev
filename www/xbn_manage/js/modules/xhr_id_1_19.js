    ...
    ctrl.on('dom-event-init',function(elem){
        ...
        $(".js-subStatus").on("click",function(){
            ...
            if(idText.indexOf("禁用") != -1){
                    ...
                    if(data.statusCode == "2000000" && data.data > 0){//如果关联了，那就不能直接禁用
                        ...
                        ctrl.request({
                            ...
                            if(data.statusCode == "2000000"){
                                ...
                                $(data.data).each(function(index, element) {
                                    html += '<li index-data="'+element.id+'"><span>'+element.groupName+'</span></li>';
                                });
                                $("#roleList").html(html);
                                sele();
                            }
                        ...
                        $.layer({
                            ...
                            dialog : {
                                ...
                                btn : ['确认', '取消'],
                                type : 8,
                                msg : '<div class="selectRole"><p>该角色存在已关联的会员。请为该类会员指定相应的角色。</p><dl class="fix"><dt>请选择角色：</dt><dd><div class="select w190 fL"><input index-data="" type="hidden" value="请选择" name="roleId"/><i>请选择</i><em class="icon-52"></em><ul id="roleList"></ul></div></dd></dl></div>',
                                yes : function(layerIndex){
                                    ...
                                    ctrl.request({
                                        ...
                                        data: JSON.stringify({
                                            "sourceId":idStr,
                                            "targetId": $("[name=roleId]").attr("index-data"),
                                            "type": 0
                                        })
                                    }).then(function(data){
                                        if(data.statusCode=='2000000'){
                        ...
                        ctrl.tirgger('setTips','确定禁用角色？',function(){
                            ctrl.request({
                                url: dataPath+ X.configer[request.m].api.disable + idStr,
                                type: "PUT",
                                data: JSON.stringify({
                ...
                ctrl.tirgger('setTips','确定启用角色？',function(){
                    ctrl.request({
